defmodule Owaygo.Location.Create do
  ...
  @all_params [:lat, :lng, :name, :discoverer]
  @required_params [:lat, :lng, :name, :discoverer]
  ...
  defp build_changeset(params) do
    ...
    |> Changeset.foreign_key_constraint(:discoverer, message: "user does not exist")
  end

  defp check_user_exists(changeset) do
    if(changeset.valid?) do
      id = changeset |> Changeset.get_change(:discoverer)
      ...
      select: count(d.id)) == 1) do
        ...
        changeset |> Changeset.add_error(:discoverer, "user does not exist")
      end
    else
      changeset
    end
  ...
  defp check_user_email_verified(changeset) do
    if(changeset.valid?) do
      ...
      |> is_nil) do
        ...
        |> Changeset.add_error(:discoverer, "email has not been verified")
      else
        changeset |> Changeset.delete_change(:email)
      end
    else
